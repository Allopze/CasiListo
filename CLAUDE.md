...
# TestableReference tienen skipped="NO"), incluidos los TRES arneses visuales:
# ScreenshotCaptureTests (23 PNG que caen en /tmp si no defines
# TEST_RUNNER_SCREENSHOT_DIR), FullLengthScreenshotTests (ImageRenderer) y
# VideoTourTests (~3,5 min de Thread.sleep; sin `simctl recordVideo` no graba).
xcodebuild test -project CasiListo.xcodeproj -scheme CasiListo \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro'

# Loop rápido: mismo comando + estos tres flags, los mismos que usa el CI.
...
- El CI ([ios.yml](.github/workflows/ios.yml)) usa `iPhone 17 Pro,OS=latest` —el mismo de aquí— y corre unitarios y UI tests en un solo `xcodebuild test`, **saltando los tres arneses visuales** (`ScreenshotCaptureTests`, `VideoTourTests` y `FullLengthScreenshotTests`, porque son diagnósticos lentos que no asertan nada): un label en español roto no es un problema local, tumba el CI del PR, pero una regresión que solo rompa las capturas pasa el CI sin ruido. Antes de los tests corre **SwiftLint con la versión fijada** en el propio workflow (`SWIFTLINT_VERSION`); los umbrales de [.swiftlint.yml](.swiftlint.yml) están calibrados contra esa versión y contra el código actual, así que hoy no hay ningún error, solo warnings (medido el 18-09-2026 con 0.65.1: **38 warnings, 0 errores**). Subir la versión exige recalibrar.
...
- Xcode local es 27.0 (Swift 6.4); el CI y el release fijan Xcode 26.2 (no 26.0: la imagen del runner ya no trae runtime de simulador iOS 26.0 y `actool` lo exige incluso en el archive para dispositivo) y solo validan `major >= 26`, así que nada avisa de la divergencia. **El único runtime instalado es iOS 27.0**, así que `--os 26.0` y cualquier prueba del piso declarado no se pueden correr aquí hasta instalar ese runtime desde Xcode.
- Release: [release-testflight.yml](.github/workflows/release-testflight.yml) es `workflow_dispatch`; **`gh` no está instalado en esta máquina** (dispáralo desde la pestaña Actions, o `brew install gh` y luego `gh workflow run release-testflight.yml --ref main`). Sube con un segundo `xcodebuild -exportArchive` usando [ci/ExportOptions-AppStoreUpload.plist](ci/ExportOptions-AppStoreUpload.plist) (`destination = upload`): Xcode 26 ya no trae `iTMSTransporter`. El primer export (`ExportOptions-AppStore.plist`) solo deja el IPA como artefacto. Secretos que espera: `ASC_KEY_ID`, `ASC_ISSUER_ID`, `ASC_PRIVATE_KEY`.
